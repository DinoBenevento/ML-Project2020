...
for train_index, test_index in skfold.split(X, y):
    ...
    history = model.fit(X[train_index], y[train_index], epochs=100, batch_size=15, verbose=2)
    ...
    print("Model " + str(index_model) + ": Test Loss: %.2f%%, Test Accuracy: %.2f%%" % (scores[0] * 100, scores[1] * 100))
    cvscores.append(scores[1] * 100)
    index_model += 1
print("Accuracy mean : %.2f%%, Standard deviation: (+/- %.2f%%), Model Variance: %.2f%%, Max Accuracy: %.2f%% " % (np.mean(cvscores), np.std(cvscores), np.std(cvscores)**2, np.max(cvscores)))
